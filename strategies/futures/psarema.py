...
from .indicators import ema,psar
class PSAREMAStrategy(BaseFuturesStrategy):
    ...
    def generate_desired_position(self,data):
        p=psar(data,self.parameters["psar_step"],self.parameters["psar_max"]);e=ema(data.close,self.parameters["ema_period"]);c=float(data.close.iloc[-1]);d=p.psar_direction.iloc[-1];ev=e.iloc[-1];diag={"psar":None if pd.isna(p.psar.iloc[-1]) else float(p.psar.iloc[-1]),"psar_direction":None if pd.isna(d) else int(d),"ema":None if pd.isna(ev) else float(ev)}
