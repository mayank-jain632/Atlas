...
class STEMAStrategy(BaseFuturesStrategy):
    ...
    def generate_desired_position(self,data):
        st=supertrend(data,period=self.parameters["supertrend_period"],multiplier=self.parameters["supertrend_multiplier"]);e=ema(data.close,self.parameters["ema_period"]);c=float(data.close.iloc[-1]);d=st.direction.iloc[-1];ev=e.iloc[-1];diag={"supertrend":None if pd.isna(st.supertrend.iloc[-1]) else float(st.supertrend.iloc[-1]),"supertrend_direction":None if pd.isna(d) else int(d),"ema":None if pd.isna(ev) else float(ev)}
